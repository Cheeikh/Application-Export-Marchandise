...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const page_cargaisons = document.querySelector(
    ".page_cargaisons"
  ) as HTMLDivElement;
  const page_produits = document.querySelector(
    ".page_produits"
  ) as HTMLDivElement;
  const page_archive = document.querySelector(".page_archive") as HTMLDivElement;
...
let products: (Alimentaire | Chimique | Fragile | Incassable)[] = [];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  cargaisonForm.addEventListener("submit", (event) => {
    ...
    const arrivalDate = calculateArrivalDate(type, distance, departureDate);
    const CargaisonID = Math.floor(
      10000000 + Math.random() * 90000000
    ).toString();
    let cargaison;
    let statut = "En attente";
    let etat = "ouverte";
    let produits: (Alimentaire | Chimique | Fragile | Incassable)[] = [];
    switch (type) {
      case "Aerienne":
...
function displayProductModal(cargaison: Aerienne | Maritime | Routiere) {
  ...
  const productWeightInput = document.getElementById(
    "product-weight"
  ) as HTMLInputElement;
  const productTypeInput = document.getElementById(
    "product-type"
  ) as HTMLSelectElement;
  const productDestinationInput = document.getElementById(
    "product-destination"
  ) as HTMLInputElement;
  ...
  const clientFirstNameInput = document.getElementById(
    "client-lastname"
  ) as HTMLInputElement;
  const clientAddressInput = document.getElementById(
    "client-address"
  ) as HTMLInputElement;
  ...
  const recipientNameInput = document.getElementById(
    "recipient-name"
  ) as HTMLInputElement;
  const recipientFirstNameInput = document.getElementById(
    "recipient-lastname"
  ) as HTMLInputElement;
  const recipientAddressInput = document.getElementById(
    "recipient-address"
  ) as HTMLInputElement;
  const recipientPhoneInput = document.getElementById(
    "recipient-phone"
  ) as HTMLInputElement;
  const recipientEmailInput = document.getElementById(
    "recipient-email"
  ) as HTMLInputElement;
  ...
  submitButton.addEventListener("click", (event: Event) => {
    ...
    const client = new Client(
      ...
      clientFirstNameInput.value.trim(),
    ...
    const destinataire = new Destinataire(
      ...
      recipientFirstNameInput.value.trim(),
    ...
    switch (type) {
      case "Alimentaire":
        ...
        break;
      case "Chimique":
        const toxicite = parseFloat(
          (document.getElementById("product-toxicite") as HTMLInputElement)
            .value
        );
        const frais = parseFloat(
          (document.getElementById("product-frais") as HTMLInputElement).value
        );
        product = new Chimique(
          productID,
          name,
          description,
          weight,
    ...
    fraisProduit = product.calculerFrais(
      cargaison.distance,
    ...
    function sendProductConfirmationEmail(
      clientEmail: string,
      ...
      if (cargaison.peutAjouterProduit(product)) {
        if (cargaison.addProduit(product)) {
          ...
          if (cargaison.getVolumeRestant() <= 0) {
            showMessageModal(
              "Cargaison pleine. Impossible d'ajouter plus de produit.",
              false
            );
          } else if (cargaison.poids + weight > cargaison.getPoidsMax()) {
            showMessageModal(
              "Poids du produit trop important. Impossible d'ajouter le produit.",
              false
            );
          } else {
            showMessageModal(
              `Produit ajouté à la cargaison. Frais : ${fraisProduit} Franc CFA`,
              true
            );
            location.reload();
          ...
          showMessageModal(
            "Le produit n'a pas pu être ajouté à la cargaison.",
            false
          );
        }
      } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function checkIfOnLand(
    latlng: L.LatLng,
    callback: (isOnLand: boolean) => void
  ) {
    fetch(
  ...
  function findNearestLandPoint(
    latlng: L.LatLng,
    callback: (nearestLandPoint: L.LatLng) => void
  ) {
...
function addProduitToCargaisonOnServer(cargaisonId, produitData) {
  fetch(
    `http://localhost/projetCargaison/dist/dataHandler.php?operationType=addProduitToCargaison&addProduitToCargaison=${cargaisonId}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
function deleteProduitFromCargaisonOnServer(cargaisonId: string, produitId: string) {
...
displayProductsInTable(cargaisons);
  ...
  .catch((error) =>
    console.error("Erreur lors de la suppression du produit de la cargaison:", error)
  );
}
...
function updateCargaisonOnServer(cargaisonId, updatedCargaisonData) {
  fetch(
    `http://localhost/projetCargaison/dist/dataHandler.php?id=${cargaisonId}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
    ...
    .then((response) => {
      ...
      if (!response.ok) {
        throw new Error(
          "Erreur du réseau lors de la mise à jour de la cargaison"
        );
      }
      return response.json();
    })
  ...
  const searchStartLocation = document.getElementById(
    "searchStartLocation"
  ) as HTMLInputElement;
  const searchEndLocation = document.getElementById(
    "searchEndLocation"
  ) as HTMLInputElement;
  ...
  async function sendEmailNotification(cargaison: any[], daysOfDelay: number): Promise<void> {
    const url = 'http://localhost/projetCargaison/sendEmail.php'; // L'URL de votre script PHP

    for (const product of cargaison) {
        const clientEmail: string = product.clientEmail;
        const destinataireEmail: string = product.destinataireEmail;
        const clientMessage: string = `Bonjour,
        Votre colis (ID: ${product.id}) a été livré avec un retard de ${daysOfDelay} jours.
        Cordialement,
        Votre Service de Livraison.`;
        
        const recipientMessage: string = `Bonjour,
...
hideCargaisonButtonsBasedOnStatus(cargaison);
  ...
  function updateCargaisonStatus(cargaisonId: string, newStatus: string, operationType: string) {
...
function applyFiltersA(cargaisons) {
  ...
  return cargaisons.flatMap((cargaison) =>
    cargaison.produits.filter((product) => {
      return (
        (!searchInput || 
...
function displayProductsInTable(cargaisons: (Aerienne | Maritime | Routiere)[]): void {
  const produitsTableBody = document.getElementById("produitsTableBody") as HTMLTableSectionElement;
  ...
  filteredProducts.forEach((product) => {
    ...
    if (cargaison && product.statut !== "Archivé") {
      ...
      const deleteIconHtml =
        cargaison.etat === "fermée" || cargaison.statut !== "En attente"
          ? ""
      ...
      if (cargaison.etat !== "fermée" && cargaison.statut === "En attente") {
        const deleteIcon = document.getElementById(deleteIconId) as HTMLElement;
...
function sendStatusUpdateEmail(cargaison, product, newStatus) {
  ...
  function removeProduct(cargaisonId, produitId) {
    ...
    const modalMessage = document.getElementById("message-text") as HTMLElement;
    const closeBtn = document.getElementById("close-message-modal") as HTMLElement;
